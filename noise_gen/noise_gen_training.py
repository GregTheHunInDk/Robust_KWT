...
def main(args):
    ...
    snr = args.snr
    #safe_path = args.sd
    data_list = data_path + "_generated/training_list.txt"
    ...
    in_list.close()
    out_list.close()
    fant_command = f'./filter_add_noise -i {current_path + noisy_path + "in.list"} -o {current_path + noisy_path + "out.list"} -n {noise_path} -u -m snr_8khz -r 1000 -d -s {snr}'
    ...
    idx = 0
    noisy_data_list = open(current_path + noisy_path + "training_list.txt", "w")
    print(current_path + noisy_path)
