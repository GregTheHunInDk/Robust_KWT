...
def main(args):
    ...
    noise_path = [
        "/home/ubuntu/noise/bbl/bbl_train.wav", 
        "/home/ubuntu/noise/bus/bus_train.wav",
        "/home/ubuntu/noise/caf/caf_train.wav",
        "/home/ubuntu/noise/str/str_train.wav"
    ]
    noise_names = ['bbl', 'bus', 'caf', 'str']
    snr = args.snr
    save_path = '/home/ubuntu/noisy_speech_commands_v0.02'
    data_list = data_path + "_generated/sorted/pretraining_list.txt"
    print(data_list)
    data_list = "/home/ubuntu/speech_commands_v0.02/_generated/sorted/pretraining_list.txt"
    ...
    noisy_path = '/pretraining_' + noise_name + '/snr_' + snr + '/'
    input_raw_path = '/pretraining_' + noise_name +  '/snr_' + snr + '/input_raw/'
    output_raw_path = '/pretraining_' + noise_name + '/snr_' + snr + '/output_raw/'
    print("################# Path info #################\n")
    ...
    # ensuring that the number of data is divisible by the number of noise type 
    # hence having uniform distribution for each noise type
    if len(datas) % len(noise_path) != 0: 
        datas = datas[:-(len(datas) % len(noise_path))]
    ...
    noise_labels = int(len(datas)/len(noise_path)) * [i for i in range(len(noise_path))]
    ...
    for in_data, out_data, noise_label in tqdm(zip(in_list.readlines(), out_list.readlines(), noise_labels)):
